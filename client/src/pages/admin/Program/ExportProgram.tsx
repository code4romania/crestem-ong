...
const getSheets = (data: Program, matrix: Matrix): Sheet[] => {
  ...
  const sheets = [{
    ...
    rows: [coverRow],
    cols: Object.keys(coverRow)
      .map((key) => ({ width: key.length + 3 }))
  }];
  ...
  data.users.forEach((user: any) => {
    ...
    user.reports.forEach((report: any) => {
      const evaluationsCompleted = evaluationsCompletedFilter(report?.evaluations);

      const scoreByEvaluation = matrix && evaluationsCompleted.length
